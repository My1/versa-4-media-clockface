...
class mononums {
  ...
  static monoDigits(num, pad = true) {
    const c0 = String.fromCharCode(0x10);
    let monoNum = '';
    if (typeof num === 'number') {
      num |= 0;
      if (pad && num < 10) {
        monoNum = c0 + this.monoDigit(num);
      } else {
        while (num > 0) {
          monoNum = this.monoDigit(num % 10) + monoNum;
          num = (num / 10) | 0;
        }
      }
    } else {
      ...
      for (let i = 0; i < textLen; i++) {
        monoNum += this.monoDigit(text.charAt(i));
      }
    }
    return monoNum;
  }
